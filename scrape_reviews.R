...
scrape_and_save_reviews <- function(number) {
  ...
  if(file.exists("data/big_scrape.rds")) {
    
    # load in the old file
    load <- readRDS("data/big_scrape.rds")
    
    # do a single scrape
    scrape <- map_dfr(pages_all$links[number], hotel_scrape)
    
    # push back together
    combine <- bind_rows(load, scrape)
    
    # save off
    saveRDS(combine,"data/big_scrape.rds")
  } else {
    # run the very first link
    big_scrape <- map_dfr(pages_all$links[1], hotel_scrape)
    
    # save off
    saveRDS(big_scrape, "data/big_scrape.rds")
  }
}
